...
function MyHooksTest(props) {
  ...
  const [number, setNumber] = useState(''); // 숫자 입력칸에 등록된 숫자 값
  const [nums, setNums] = useState([]); // 입력한 숫자들을 들고 있을 배열(정보가 쌓여야하는 곳이라 배열)

  // ============================================================
  // 변수인데, 값이 변경되어도 랜더링 하지 않는다, 내부 연산용, 화면에 상관없는 용도로 변수 사용
  // - `랜더링이 관여X`
  ...
  // 화면이 갱신될때마다 연산이 매번 수행된다 => 연산에 관여된 변수들 중에 변화가 없다면 연산 미수행
  // useMemo
  // nums 상태변수가 변경되면 -> 이를 감지해서 -> 콜백이 호출되고 -> 연산이 수행되어서 -> 화면갱신을 한다.
  const avg = useMemo(() => {
  ...
  const onChangeNumber = useCallback((e) => {
    ...
    setNumber(v);
  }, []); // 컴포넌트가 최초 랜더링될 때 함수를 생성( 1회만 수행, 최초에 한 번 만들고 끝)
  const onRegi = useCallback(
    (e) => {
      // number 상태변수가 가지고 있는 값을 nums에 추가한다.
      // .push는 일반 값으로 들어가고 .concat은 배열로 들어간다.
